...
- **Original Release Year Lookup**: To prevent remastered or compilation album dates (e.g. Journey's *Don't Stop Believin'* showing 2001 instead of 1981), the app queries the **MusicBrainz API** on-the-fly using programmatically generated double-quoted title variations joined by `OR` (minimizing query pollution and preventing regressions) to extract the earliest release date. It cascades through candidate-filtered `limit=10` search results from the **iTunes Search API** (via local dev proxy, `proxy.php`, or AllOrigins proxy) to guarantee the true original release year, falling back to the **Deezer API** track metadata if needed.
