...
export interface GameState { lang: Language; theme: Theme; players: Player[]; currentPlayerIndex: number; currentRound: number; totalRounds: number; phase: GamePhase; songPool: Song[]; currentSong: Song | null; currentStep: QuestionStep; turnPoints: number; turnResults: { title: number; artist: number; year: number; }; history: Song[]; genres: string[]; }